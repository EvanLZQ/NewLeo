...
def google_login(request):
    ...
    response.set_cookie('access_token', token.token,
                        max_age=3600 * 24, samesite='Lax', domain='.eyelovewear.com', path='/api/user/', httponly=True, secure=True)  # 1 day
    response.set_cookie('refresh_token', refresh_token.token,
                        max_age=3600 * 24 * 30, samesite='Lax', domain='.eyelovewear.com', path='/api/user/', httponly=True, secure=True)  # 30 days
